...
class pnn_classifier:
    ...
    def pnn_algorithm(self):
        ...
        for i in range(1, len(self.num_class)):
    ...
    def performance_analysis(self):
        ...
        confusion_matrix = np.zeros((matrix_dim, matrix_dim), dtype=np.int)
        actual_class = self.test_data['Label'].tolist()
        actual_class = [str(x) for x in actual_class]
        test_group_len = self.train_data.groupby('Label')
        test_group_len = [len(test_group_len.get_group(key)) for key, value in test_group_len]
        ...
        total_false_negative = np.subtract(test_group_len, total_true_positive)
        print('Total False Negative = ', total_false_negative)

        total_false_positive = np.sum(confusion_matrix.T, axis=0)
        ...
        plt.plot(roc_fpr, roc_recall, color='red', marker='o', markerfacecolor='orange', markersize=12 )
...
initialize = pnn_classifier("letter-recognition.csv" , "letter-recognition_test.csv")
